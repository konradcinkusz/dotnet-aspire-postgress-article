...
<!-- src/HobbitGame.Web/HobbitGame.Web.csproj (additions only) -->
<ItemGroup>
...
<ItemGroup>
  <ProjectReference Include="..\HobbitGame.ServiceDefaults\
HobbitGame.ServiceDefaults.csproj" />
</ItemGroup>
\end{lstlisting}
...
// src/HobbitGame.Web/Program.cs (additions only; existing lines unchanged)
var builder = WebApplication.CreateBuilder(args);

// --- Aspire additions ---
builder.AddServiceDefaults();                             // (1)
builder.AddNpgsqlDbContext<AppDbContext>("hobbitdb");      // (2)
...
\textbf{(2) \texttt{AddNpgsqlDbContext\textless AppDbContext\textgreater("hobbitdb")}.} Registers \texttt{AppDbContext} in the DI container, reads the connection string from \texttt{ConnectionStrings\_\_hobbitdb}, and enables Aspire health checks and telemetry for the database connection. The string \texttt{"hobbitdb"} must match the argument passed to \texttt{AddDatabase} in the AppHost. No \texttt{appsettings.json} entry is needed.
...
For the HobbitGame POC, schema creation uses \texttt{EnsureCreatedAsync}, which creates tables if they do not exist and is a no-op if they do. This is the appropriate choice for local development without a formal migration history. For production or migration-tracked environments, \texttt{MigrateAsync} should be used instead~\cite{efcoredocs2024}:
