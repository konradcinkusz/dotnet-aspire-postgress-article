...
The AppHost \texttt{Program.cs} consists of three logical blocks: (i) builder creation, (ii) resource declarations, and (iii) build-and-run. The complete canonical file for the HobbitGame scenario is:

\begin{lstlisting}[language={[Sharp]C}]
// src/HobbitGame.AppHost/Program.cs
...
var hobbitDb = postgres.AddDatabase("hobbitdb");

builder.AddProject<Projects.HobbitGame_Web>("webapp")
    .WithReference(hobbitDb)
    .WaitFor(hobbitDb)
...
\addlinespace
\texttt{.AddDatabase("hobbitdb")} & Registers logical DB; name must match \texttt{AddNpgsqlDbContext<>} argument & \texttt{ConnectionStrings\_\_hobbitdb} not injected; EF cannot resolve context \\
\addlinespace
\texttt{.WithReference(hobbitDb)} & Injects \texttt{ConnectionStrings\_\_hobbitdb} env var into web app & App starts without connection string; runtime exception on first DB call \\
\addlinespace
\texttt{.WaitFor(hobbitDb)} & Delays web app launch until Postgres health check passes & Race condition; app may start before DB is ready \\
\addlinespace
...
The string argument to \texttt{AddDatabase} --- \texttt{"hobbitdb"} --- determines the key under which the connection string is injected into the web project's environment. Aspire injects it as \texttt{ConnectionStrings\_\_hobbitdb} (using double underscore as the hierarchy separator, consistent with the .NET configuration system). The \texttt{Aspire.Npgsql.EntityFrameworkCore.PostgreSQL} package reads this key automatically when \texttt{AddNpgsqlDbContext\textless AppDbContext\textgreater("hobbitdb")} is called in the web project's \texttt{Program.cs}. The two strings must match exactly; a mismatch produces a runtime \texttt{InvalidOperationException} with a message indicating that no connection string named \texttt{"hobbitdb"} was found.
...
\texttt{WaitFor(hobbitDb)} ensures the web application process does not start until the Postgres container passes its TCP health check. This eliminates the most common race condition. However, it does not wait for the \textit{logical} database to exist --- only for the Postgres process to be accepting connections. If the web application calls \texttt{dbContext.Database.EnsureCreatedAsync()} or \texttt{MigrateAsync()} at startup, those calls will execute after the container is ready, which is the correct ordering. No additional delay or retry logic is required in the web project.
